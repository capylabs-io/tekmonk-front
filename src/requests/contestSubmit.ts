...
export const getOneContestSubmission = async (id: string) => {
    const response = await tekdojoAxios.get(`${BASE_URL}/contest-submissions/${id}?populate[contest_entry][populate]=user&populate[thumbnail]=*&populate[assets]=*&populate[source]=*`);
    return response.data;
}

export const getContestSubmissionByContestEntry = async (contestEntryId: string) => {
    const response = await tekdojoAxios.get(`${BASE_URL}/contest-submissions?populate[contest_entry][populate]=user&populate=thumbnail&populate[assets]=*&filters[contest_entry][id][$eq]=${contestEntryId}`);
    return response.data;
}

export const getContestSubmissionResult = async (page: number, limit: number, name: string, candidateNumber: string, project: string) => {
    const response = await tekdojoAxios.get(`${BASE_URL}/contest-submissions?populate[contest_entry][populate]=user&pagination[page]=${page}&pagination[pageSize]=${limit}&sort[1]=id:asc&sort[0]=QualifiedExam:asc&filters[contest_entry][candidateNumber][$containsi]=${candidateNumber}&filters[contest_entry][user][fullName][$containsi]=${name}&filters[title][$containsi]=${project}`); 
    return response.data;
};