...
export const AvatarLayer = ({ avatarConfig, customClassName }: Props) => {
  return (
    <div className={cn("border-2 border-gray-30 bg-[rgb(245,245,245)] relative overflow-hidden", customClassName)}>
      ...
      {avatarConfig?.mouth && <Image src={avatarConfig?.mouth?.image || ''} alt={avatarConfig?.mouth?.name || ''} fill className={cn("object-cover absolute z-[4]")} />}
      {avatarConfig?.eye && <Image src={avatarConfig?.eye?.image || ''} alt={avatarConfig?.eye?.name || ''} fill className={cn("object-cover absolute z-[3]")} />}
      {avatarConfig?.theme && <Image src={avatarConfig?.theme?.image || ''} alt={avatarConfig?.theme?.name || ''} fill className={cn("object-cover absolute z-[1]")} />}
      {avatarConfig?.special && <Image src={avatarConfig?.special?.image || ''} alt={avatarConfig?.special?.name || ''} fill className={cn("object-cover absolute z-[5]")} />}
    </div>
  )
}