    ...
    <div className="px-6 mt-3">
      ...
      <div className="flex flex-col mt-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {myClassList.length > 0 && myClassList.map((classItem) => {
            return (
              <CommonCard
                key={classItem.id}
                ...
                <div className="flex flex-col items-start justify-between h-full w-full">
                  ...
                  <div className="space-y-3 w-full">
                    <div className="flex items-center text-BodySm text-gray-600 bg-gray-100 p-2 rounded-lg">
                      <span className="font-medium text-gray-700 mr-2">Khoá học:</span>
                      <span className="text-gray-95">{get(classItem, "course.name", "")}</span>
                    </div>

                    <div className="flex items-center justify-between">
                      <div className="flex items-center text-BodySm">
                        <span className="font-medium text-gray-700 mr-2">Trạng thái:</span>
                        <span className={`px-3 py-1 rounded-full text-sm ${new Date(get(classItem, "endTime", "")).getTime() > new Date().getTime()
                          ? "bg-green-100 text-green-700"
                          : "bg-gray-100 text-gray-700"
                          }`}>
                          {new Date(get(classItem, "endTime", "")).getTime() > new Date().getTime() ? "Đang diễn ra" : "Đã kết thúc"}
                        </span>
                      </div>
                    </div>
                  </div>
