            ...
            <hr className='border-t border-gray-200 my-4' />
            {
                listEvent?.map((event, index) => {
                    return (
                        <>
                            <div key={event?.title}>
                                <EventCard day={event?.day} month={event?.month} weekday={event?.weekday} timeRange={event?.timeRange} title={event?.title} />
                            </div>
                            {index + 1 !== listEvent.length && <hr className='border-t border-gray-200 my-4' />}
                        </>
