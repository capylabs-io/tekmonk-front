        ...
        <div>
                ...
                <div className='mt-5 flex gap-x-5 overflow-hidden'>
                    {
                        AvatarList.map((avatar, index) => {
                            return (
                                <AvatarCard imageUrl={avatar.image} title={avatar.title} price={avatar.price} key={avatar.title} />
                            )
                        })
                    }
                </div>
            ...
            <div className="text-primary-900 px-4 mt-2">
                <div className='w-full flex justify-between'>
                    ...
                    <ArrowRight className='text-gray-600 cursor-pointer' size={24} onClick={() => router.push('/shop/background')} />
                </div>
                <div className='mt-5 flex gap-x-5 overflow-hidden'>
                    {
                        slicedBackgroundList.map((background, index) => {
                            return (
                                <BackgroundCard imageUrl={background.image} title={background.title} price={background.price} key={background.title} />
                            )
                        })
                    }
                </div>
